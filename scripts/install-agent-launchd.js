...
const LABEL = "com.pikiio.pq-agent";
const PLIST_PATH = path.join(os.homedir(), "Library/LaunchAgents", `${LABEL}.plist`);
const LOG_DIR = path.join(ROOT_DIR, "logs");
const NODE_BIN = process.execPath;
const WORKER_PATH = path.join(ROOT_DIR, "scripts/pq-agent-worker.js");
...
<dict>
  ...
  <string>${xml(path.join(LOG_DIR, "agent-worker.log"))}</string>
  <key>StandardErrorPath</key>
  <string>${xml(path.join(LOG_DIR, "agent-worker.err.log"))}</string>
...
try {
  ...
  else throw new Error("Usage: node scripts/install-agent-launchd.js install|uninstall|status");
} catch (error) {
