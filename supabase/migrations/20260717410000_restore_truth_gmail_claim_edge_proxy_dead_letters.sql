-- Restore claim producers whose final failure was the edge HTTP proxy
-- (Cloudflare 520) severing the long-running append-and-seal RPC before
-- the 300s statement budget could matter. This is retry authority, never
-- evidence exclusion: the worker must still execute and seal its real
-- candidate set before acceptance can advance. The companion repair is
-- client-side: the worker pre-appends candidates in bounded statements so
-- the final atomic call carries only the seal (no single request can
-- outlive the proxy's ~100s ceiling).
...
begin
  if to_regclass('public.source_processing_jobs') is null
    ...
    raise exception 'Gmail claim edge-proxy retry prerequisites are unavailable'
      using errcode='55000';
  end if;
end;
$preflight$;

create table if not exists public.truth_gmail_claim_edge_proxy_retry_authorizations (
  authorization_id text primary key check (
    authorization_id='truth-gmail-claim-edge-proxy-retry:v1:'||authorization_hash
  ),
  ...
  prior_error_code text not null check (prior_error_code='TRUTH_CANDIDATE_RPC_FAILED'),
  ...
  schema_version text not null check (
    schema_version='truth-gmail-claim-edge-proxy-retry-authorization-v1'
  ),
  ...
  check (canonical_authorization->>'reasonCode'='CLAIM_APPEND_SEAL_EDGE_PROXY_RETRY'),
  check (canonical_authorization->>'productionPublicationAttempted'='false')
);

drop trigger if exists truth_gmail_claim_edge_proxy_retry_authorizations_immutable
  on public.truth_gmail_claim_edge_proxy_retry_authorizations;
create trigger truth_gmail_claim_edge_proxy_retry_authorizations_immutable
before update or delete on public.truth_gmail_claim_edge_proxy_retry_authorizations
for each row execute function public.reject_immutable_truth_mutation();
alter table public.truth_gmail_claim_edge_proxy_retry_authorizations enable row level security;
alter table public.truth_gmail_claim_edge_proxy_retry_authorizations force row level security;
revoke all on table public.truth_gmail_claim_edge_proxy_retry_authorizations
  from public,anon,authenticated,service_role;
grant select on table public.truth_gmail_claim_edge_proxy_retry_authorizations
  to service_role;

do $restore$
declare
  v_detail constant text :=
    'append and seal candidate claim job failed: Supabase RPC failed: append_and_seal_candidate_claim_job 520';
...
begin
  ...
  for v_job in
    ...
    where job.workspace_key='primary'
      ...
      and job.last_error_code='TRUTH_CANDIDATE_RPC_FAILED'
      and job.safe_error_detail=v_detail
      and job.lease_owner is null and job.lease_expires_at is null
      and job.result='{}'::jsonb
      and not exists(
        select 1
        from public.truth_gmail_claim_edge_proxy_retry_authorizations auth
  ...
  loop
    ...
    v_body:=jsonb_build_object(
      'schemaVersion','truth-gmail-claim-edge-proxy-retry-authorization-v1',
      ...
      'reasonCode','CLAIM_APPEND_SEAL_EDGE_PROXY_RETRY',
      'productionPublicationAttempted',false
    );
    v_hash:=encode(extensions.digest(convert_to(
      private.truth_canonical_json_text(v_body),'UTF8'),'sha256'),'hex');
    v_id:='truth-gmail-claim-edge-proxy-retry:v1:'||v_hash;
    insert into public.truth_gmail_claim_edge_proxy_retry_authorizations(
      ...
      'truth-gmail-claim-edge-proxy-retry-authorization-v1'
    );
    ...
    set state='retry_wait',max_attempts=v_max,available_at=clock_timestamp(),
      ...
      last_error_code='TRUTH_CLAIM_EDGE_PROXY_RETRY_AUTHORIZED',
      safe_error_detail=
        'The prior claim append/seal attempt was severed by the edge HTTP proxy; bounded decomposed retry is authorized.',
    ...
    if v_updated<>1 then
      raise exception 'Gmail claim edge-proxy retry fence changed'
        using errcode='40001';
    end if;
  end loop;
end;
...
begin
  if exists(
    ...
    raise exception 'Gmail claim edge-proxy retry authorization is incomplete'
      using errcode='23514';
  end if;
end;
$verify$;
