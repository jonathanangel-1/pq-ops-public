-- TMS active-inventory snapshots must be readable by the token-gated RPCs.
-- Gmail refresh writes tms-detail-snapshot and tms-grid-snapshot, and truth
-- health requires their metadata to certify that the active universe came
-- from the current CourierCloud/TMS scrape.
