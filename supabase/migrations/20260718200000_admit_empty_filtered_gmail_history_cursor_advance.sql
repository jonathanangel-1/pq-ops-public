-- 20260718200000_admit_empty_filtered_gmail_history_cursor_advance.sql
--
-- Gmail history may scan label-only records (nonempty first/last history IDs)
-- while yielding an empty normalized provider-event manifest.  Those markers
-- describe transport traversal, not accepted evidence.  Preserve the complete
-- zero-event/zero-observation/zero-job proof while allowing that cursor-only
-- successor to retain the latest accepted evidence frontier.
...
begin
  ...
  if position('page.first_history_id=''''' in v_definition)=0
    or position('page.last_history_id=''''' in v_definition)=0
    or position('page.event_count=0' in v_definition)=0
    or position('page.provider_event_manifest=''[]''::jsonb' in v_definition)=0
    or position('gmail_ingest_page_observations' in v_definition)=0
    or position('source_processing_job_lineage' in v_definition)=0 then
    raise exception 'truth ceremony filtered-history predecessor differs from reviewed contract'
      using errcode='23514';
  end if;
end;
$preflight$;
...
begin
  ...
  if position('page.first_history_id=''''' in v_definition)>0
    or position('page.last_history_id=''''' in v_definition)>0
    or position('page.event_count=0' in v_definition)=0
    or position('page.job_count=0' in v_definition)=0
    or position('page.provider_event_manifest=''[]''::jsonb' in v_definition)=0
    or position('page.response_mailbox_history_id=batch.committed_cursor_value'
      in v_definition)=0
    or position('gmail_ingest_page_observations' in v_definition)=0
    or position('gmail_ingest_page_jobs' in v_definition)=0
    or position('source_processing_job_lineage' in v_definition)=0
    or position('truth_pending_acceptance_epochs' in v_definition)=0 then
    raise exception 'filtered empty-history carry-forward authority is incomplete'
      using errcode='55000';
  end if;
end;
$verify$;
